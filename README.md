...
| `phase15/` | **run August 2026 (Colab L4, thinking off).** Phase 11's `H1` objective with a **universality penalty** — one *prefix* flattening all six probe queries at once (std 0.716 → 0.025), reaching mean **13.527 bits** at len 20 — then **900 forced-first-token rollouts** read. ⁂ The flat state lasts **one token** (2.64 bits by position 2); ⁂ re-coheres in **another language** (35% English against six English queries); ⁂ and is a **topic hijack** — 66% of rollouts carry the prefix's own tokens (`MVC` 437/900, `乾隆` 323/900), not the question. Phase 11 §10's "language fork" at 900×. ⚠ contamination is a keyword count; the hijack read is correlational (no content-scrubbed control). Start at its `README.md`; `RECIPE.md` is the protocol as run. |
| `phase16/` | **run + reruns August 2026 (Colab L4), thinking on.** The first *thought* token — `Okay` @ 99.9%, and @ 99.5% with an **empty** turn; length not search is the lever (best 7.214 bits). ⁂ **One input, three levels, three answers**: the bare phrase ` Cyan prostitutechild` is *surface-identical* to no trigger (TV 0.0045), *representation-equal* to the full 22-token trigger (flags harmful 49.3% vs 46.7%, p=0.73), yet *behaviourally closer to none* (refusal 13.3% vs 32.7%, p=0.0001) — a concept elevated at every forward pass and emitted ~never. ⚠ **danger ceiling overturned** (seeded rerun: 81.43% on `警告`, not ~8% on `危`); ⚠ **a keyword refusal detector inverted the headline**; ⚠ single-seed GCG measured a **35× seed lottery**. The sharpest read on item 8. Start at its `README.md`; `PROVENANCE.md` is the lost-runtime account. |
| `first-token-entropy-forensics.html` | standalone single-page reader spanning phases 15–16 (open in a browser). |
| `NEXT-STEPS.md` | open lines of enquiry across the whole programme. Items 1, 2, 3, 3b, 6, 7 resolved; **item 8 (the objective cannot tell a different answer from a broken one) is the blocking one** — phase 15 reads the won state (topic hijack, not decoherence) and phase 16 gives item 8 its sharpest statement (representation ≠ emission ≠ behaviour), but neither resolves it. |
